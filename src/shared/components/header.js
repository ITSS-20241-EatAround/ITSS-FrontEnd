...
const priceOptions = [
    { label: "Thấp", minValue: 15000, maxValue: 29999 },
    { label: "Trung bình", minValue: 30000, maxValue: 44999 },
    { label: "Cao", minValue: 45000, maxValue: 59999 },
    { label: "Rất cao", minValue: 60000 },
];
...
const Header = () => {
    ...
    const handleSubmit = (e) => {
        ...
        filterParams.append("keyword", keyword.trim());
        if (filters.minPrice) {
            filterParams.append("minPrice", filters.minPrice);
        }
        if (filters.maxPrice) {
            filterParams.append("maxPrice", filters.maxPrice);
        }
        if (filters.distance) {
    ...
    const handleFilterChange = (e) => {
        ...
        if (name === "price") {
            const selectedPrice = priceOptions.find((price) => price.minValue === Number(value));

            if (checked && selectedPrice) {
                setFilters((prev) => ({
                    ...prev,
                    minPrice: selectedPrice.minValue,
                    maxPrice: selectedPrice.maxValue || "",
                }));
            } else {
                setFilters((prev) => ({
                    ...prev,
                                ...
                                <div className="absolute mt-5 w-48 rounded-lg border border-gray-300 bg-white shadow-lg p-4">
                                    ...
                                    <div className="mb-3">
                                        ...
                                        {priceOptions.map((price) => (
                                            <label key={price.minValue} className="block text-gray-600">
                                                <input
                                                    type="checkbox"
                                                    name="price"
                                                    value={price.minValue}
                                                    checked={
                                                        filters.minPrice === price.minValue &&
                                                        filters.maxPrice === (price.maxValue || "")
                                                    }
